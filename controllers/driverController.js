...
export const resendDriverOTP = async (req, res) => {
  try {
    ...
    const driver = await Driver.findOne({
      $or: [{ email: email?.toLowerCase() }, { _id: driverId }],
    });
...
export const updateDriverProfile = async (req, res) => {
  try {
    ...
    const { fullName, phoneNo, vehicleInfo, licenseNumber, profileImage } =
      req.body;
...
export async function forgotPassword(req, res) {
  try {
    ...
    driver.resetPasswordExpire = Date.now() + 10 * 60 * 1000; // 10 minutes

    await driver.save();

    // Create reset URL
    const resetUrl = `${process.env.FRONTEND_URL}/auth/reset-password/${resetToken}?type=driver`;

    try {
      await sendPasswordResetEmail(driver.email, driver.fullName, resetUrl);
