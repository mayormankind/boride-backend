...
export async function registerStudent(req, res) {
  try {
    ...
    if (!isValidMatricNumber(matricNo)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid matric number format" });
    }
...
export async function resendVerificationOTP(req, res) {
  try {
    ...
    const student = await Student.findOne({
      $or: [{ email: email?.toLowerCase() }, { _id: studentId }],
    });
...
export async function forgotPassword(req, res) {
  try {
    ...
    // Create reset URL
    const resetUrl = `${process.env.FRONTEND_URL}/auth/reset-password/${resetToken}?type=student`;

    try {
      await sendPasswordResetEmail(student.email, student.fullName, resetUrl);
